...
1. 函数不是一个过程，而是一个合约
    ...
    > 使用合约这个词是参考了区块链的智能合约技术，它可以将一块代码作为区块保存，并使其不可变。
    >
    > 同样的，在冯诺伊曼体系中，很重要的一块就是代码和数据共同储存。
    >
    > 函数式编程在某种程度上是在更小的问题范围内实现这个思路。
    >
    > 更进一步，react 生命周期方法也是在恰当的时候被调用的一份合约。
...
1. 在 react 中使用函数式编程
    ...
    类式的声明法，最大的区别在于生命周期函数。那么生命周期函数也应该遵循无状态和无副作用的原则。

    但是事实上，有很多情况下我们会有状态，这是声明式的另一个区别，即可以拥有自己的 state。

1. 提升 state

    props 和 state 引起的组件行为是一致的，这是 react 核心团队仔细考虑后的结果，这使得提升 state 是简单的。

    除非 state 是组件本身独有的，并且组件是为了重用而存在的，我们应当使用 state。

    除此之外，出于一些理由，我认为在其他情况下我们应该提升 state。把它放到 redux 然后 connect，以 props 的形式作用于组件（如果它确实需要作用于组件）

1. 为什么提升 state，并使用函数式编程

    1. 没有 state 的组件是简单的。

    1. 如果多个组件公用一个 state，我们没有什么理由说 state 属于其中的某一个（通常会是父亲）。
...
现在我们只考虑容器类组件，它们通常是 view 层的逻辑的部分，是视图的骨架，对于容器类组件的最佳实践是什么？这是一个尚未有定论的问题。
...
1. 只要是不可变的部分都可以被抽出
    ...
    编程是不停地勾勒逻辑的边界，画出细致的轮廓线。我们可以用函数来表达一个数据，也可以用数据表达一个函数。这不影响代码块之间的耦合性。
...
1. dva
    ...
    先说 dva，把 action, saga 和 reducer 写在一个 module 下，成为一个巨大的对象，并没有得到什么好处。把它们单纯的文件分割，单一职责的原则都会实践得更好，更不说 module 模式本身有无法适应的情况。
...
1. redux-saga
    ...
    redux-saga 还提供了很多 effect，takeLatest 之类，它们可以做成一个新的库，暂时还没有找到以这个思路出发的开源项目，如果现有的代码大量依赖这些 effect，可能成为放弃 redux-saga 的阻力。
...
1. mixin => hoc
    ...
    1. 继承或 mixin，比如继承 PureComponent 得到一个 通用的 shouldComponentUpdate 方法，甚至实现多继承。
